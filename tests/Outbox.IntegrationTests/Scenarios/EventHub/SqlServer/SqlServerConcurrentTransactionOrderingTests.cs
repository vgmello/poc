...
/// <summary>
///     SQL Server + EventHub variant of <see cref="ConcurrentTransactionOrderingTests" />.
...
public class SqlServerConcurrentTransactionOrderingTests
{
    ...
    public async Task FetchBatch_WithUncommittedRowOnDifferentPartitionKey_ShouldStillReturnOtherPartitions()
    {
        ...
        var txn1Task = Task.Run(async () =>
        {
            ...
            await InsertInTransactionAsync(conn, tx, eventHub, "slow-key",
                eventIndex: 0, timestamp: TimestampA);
        ...
        await SqlServerTestHelper.InsertMessagesAsync(
            _infra.SqlServerConnectionString, 3, eventHub, "fast-key", startOrdinal: 0);
