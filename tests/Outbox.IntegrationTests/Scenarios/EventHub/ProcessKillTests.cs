...
public class ProcessKillTests
{
    ...
    public async Task SigKill_SurvivingPublisher_ClaimsOrphanedPartitions_AndDrainsMessages()
    {
        ...
        // Start publisher A
        var (hostA, transportA) = EventHubTestHelper.BuildEventHubPublisherHost(
        ...
        // Start publisher B
        var (hostB, transportB) = EventHubTestHelper.BuildEventHubPublisherHost(
            _infra.ConnectionString, _infra.EventHubConnectionString);
    ...
    public async Task SigKill_MessagesPickedUpByNewPublisher_WithFreshAttemptBudget()
    {
        ...
        try
        {
            ...
            // Verify messages ended up in EventHub (published) or dead letter
            var consumed = await EventHubTestHelper.ConsumeMessagesAsync(
                _infra.EventHubConnectionString, eventHub, 5, TimeSpan.FromSeconds(10), partitionKey);
