...
public class RebalanceOrderingTests
{
    ...
    {
        var eventHub = EventHubTestHelper.CheckoutNamedHub("test-hub-rebalance-16p");
        ...
        await OutboxTestHelper.InsertMessagesAsync(
            _infra.ConnectionString, messageCount, eventHub, partitionKey, startOrdinal: 0);
        ...
        try
        {
            ...
            var consumed = await EventHubTestHelper.ConsumeMessagesAsync(
                _infra.EventHubConnectionString, eventHub, messageCount, TimeSpan.FromSeconds(15), partitionKey);
    ...
    public async Task MultiplePartitionKeys_OrderingPreservedPerKeyWithConcurrentPublishers()
    {
        ...
        try
        {
            ...
            var partitionKeys = Enumerable.Range(0, keysCount)
                .Select(i => $"pk-{testId}-{i}")
                .ToList();

            foreach (var key in partitionKeys)
            {
                await OutboxTestHelper.InsertMessagesAsync(
                    _infra.ConnectionString, messagesPerKey, eventHub, key, startOrdinal: 0);
            }
            ...
            {
                var consumed = await EventHubTestHelper.ConsumeMessagesAsync(
                    _infra.EventHubConnectionString, eventHub, messagesPerKey, TimeSpan.FromSeconds(15), key);

                var indices = ExtractIndices(consumed);
                var deduped = Deduplicate(indices);

                _output.WriteLine($"  Key '{key}': {consumed.Count} consumed, {deduped.Count} unique");
    ...
    public async Task CrashRecovery_OrderingPreservedAfterPublisherTakeover()
    {
        ...
        try
        {
            ...
            // Consume ALL messages from EventHub and verify ordering.
            ...
            var consumed = await EventHubTestHelper.ConsumeMessagesAsync(
                _infra.EventHubConnectionString, eventHub, messageCount * 2, TimeSpan.FromSeconds(15), partitionKey);
    ...
    public async Task ContinuousInsertion_OrderingPreservedDuringRebalance()
    {
        ...
        var insertionTask = Task.Run(async () =>
        {
            ...
            while (!insertionCts.Token.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@topic", eventHub);
        ...
        try
        {
            ...
            try
            {
                ...
                // Consume and verify ordering
                var consumed = await EventHubTestHelper.ConsumeMessagesAsync(
                    _infra.EventHubConnectionString, eventHub, totalInserted, TimeSpan.FromSeconds(15), partitionKey);
    ...
    public async Task ConcurrentTransactions_OrderingPreservedThroughPublisherPipeline()
    {
        ...
        var (host, _) = EventHubTestHelper.BuildEventHubPublisherHost(
            _infra.ConnectionString, _infra.EventHubConnectionString);
        ...
        try
        {
            ...
            var prematureMessages = await EventHubTestHelper.ConsumeMessagesAsync(
                _infra.EventHubConnectionString, eventHub, 1, TimeSpan.FromSeconds(2), partitionKey);
            ...
            // Consume from EventHub and verify ordering
            var consumed = await EventHubTestHelper.ConsumeMessagesAsync(
                _infra.EventHubConnectionString, eventHub, 3, TimeSpan.FromSeconds(10), partitionKey);
    ...
    private static List<int> ExtractIndices(IEnumerable<ConsumedEventHubMessage> messages)
    {
        return messages
            .Select(m =>
            {
