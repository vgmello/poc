...
**What we're testing:** BUG-5 fix — on graceful shutdown, in-flight leases are released immediately (not waiting for lease expiry), reducing message processing delay.
...
3. **Assert:** `UnregisterProducerAsync` is called
4. **Assert:** Any in-flight leased messages have their leases released (leased_until_utc = NULL)
...
**What we're testing:** BUG-2 fix — a message that consistently fails transport send (e.g., too large for EventHub batch) eventually gets dead-lettered instead of retrying infinitely.
...
5. **Assert:** On next lease, message identified as poison (`retry_count >= MaxRetryCount`)
6. **Assert:** Message moved to `outbox_dead_letter` table
...
**Verify no infinite retry (regression test for BUG-2):**
- Track `retry_count` after each lease-release cycle
- Must increment by 1 each time (because `incrementRetry: true`)
- Must NOT stay at 0 forever (old behavior)

---

## Scenario 6: Intermittent Transport Failures

**What we're testing:** BUG-2 fix — under intermittent failures (2 fail, 1 success pattern), messages still eventually dead-letter instead of retrying forever.
...
- OLD: `retry_count` never incremented on explicit release → infinite retry
- NEW: `retry_count` increments on transport failure → dead-letter after `MaxRetryCount`
...
5. **Assert:** Messages that C had leased are released on graceful shutdown (BUG-5 fix)
...
## Scenario 9: Loop Crash and Auto-Recovery (BUG-1 Fix)
...
7. **Assert:** On next successful poll, `ConsecutiveLoopRestarts` resets to 0
...
7. **Assert:** `retry_count` increments on each failed send (BUG-2 fix)
