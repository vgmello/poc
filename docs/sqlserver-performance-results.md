...
The FetchBatch query uses an ordered Index Seek on `IX_Outbox_Pending (PartitionId, SequenceNumber)` — a fully covering nonclustered index. The `ORDER BY` clause matches the index key order exactly, eliminating any Sort operator. No missing index suggestions.

> **Note:** The throughput numbers in this report were measured against the old `(PartitionId, EventDateTimeUtc, EventOrdinal)` index. A re-baseline run on the current index is pending.
