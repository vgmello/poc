...
- Multi-hub support: a single transport instance MAY handle multiple hubs. The Azure SDK requires one `EventHubProducerClient` per hub, so the transport caches clients per `topicName` internally. Transport-level options (`MaxBatchSizeBytes`, `SendTimeoutSeconds`) are shared across all hubs served by the same transport instance — register multiple transports via named groups if per-hub tuning is required.
...
1. **FetchBatch:** MUST return messages such that rows sharing a `(topic, partition_key)` are returned in `sequence_number` order (equals insert order). The SQL `ORDER BY` clause is store-specific and chosen to match the store's index layout — PostgreSQL uses `ORDER BY sequence_number` (index `ix_outbox_pending (sequence_number)`); SQL Server uses `ORDER BY PartitionId, SequenceNumber` (composite nonclustered index `IX_Outbox_Pending (PartitionId, SequenceNumber)`). Both satisfy the ordering invariant because rows sharing a `partition_key` share a `partition_id` by construction, so per-key ordering is preserved under either sort. Swapping SQL Server to a bare `ORDER BY SequenceNumber` forces a clustered-PK scan with per-row partition lookups (measured ~48× more logical reads on a 50k-row table) and must not be done. MUST apply the version ceiling filter (`xmin < pg_snapshot_xmin(pg_current_snapshot())` on PostgreSQL, `RowVersion < MIN_ACTIVE_ROWVERSION()` on SQL Server) to withhold rows from in-flight write transactions. MUST only return messages for partitions owned by the requesting publisher (with grace period check). MUST return `payload_content_type` alongside `headers`/`payload`. MUST deserialize `headers` from JSON text to `Dictionary<string, string>?`. This is a pure SELECT — no row locking or UPDATE is performed. **SQL Server: the query MUST use `WITH (NOLOCK)` on the outbox table.** Without it, `READ COMMITTED` takes shared locks during scanning and blocks on rows with exclusive locks from uncommitted transactions, causing timeouts. `NOLOCK` and `MIN_ACTIVE_ROWVERSION()` work together by design: `NOLOCK` prevents reader blocking on writer locks, while `MIN_ACTIVE_ROWVERSION()` filters out uncommitted rows by their row version. Do not remove `NOLOCK` — it is required for non-blocking reads.
